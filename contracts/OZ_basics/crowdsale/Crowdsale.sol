...
contract CS_ERC777Token{
    ...
    function set_campaign_address(address campaign_address) public;
...
contract Crowdsale is ReentrancyGuard {
     ...
     */
    function buyTokens(address payable beneficiary) public nonReentrant payable {
        ...
        (token_bits, tokens) = _getTokenAmount(weiAmount);

        //calculate if refunded is needed
        uint256 refund_amount = weiAmount.sub(tokens.mul(_rate));

        weiAmount = weiAmount.sub(refund_amount);

        // update state
        _weiRaised = _weiRaised.add(weiAmount);

        _processPurchase(beneficiary, token_bits, refund_amount);
    ...
    function _processPurchase(
        ...
        internal {
        // if(refund_amount > 0 ){
        beneficiary.transfer(refund_amount);
        // }
    ...
    internal view returns (uint256, uint256)
  {
    ...
    uint tokens = weiAmount.div(_rate);
    uint token_bits = tokens.mul(1 ether);
    return (token_bits, tokens);
  }
